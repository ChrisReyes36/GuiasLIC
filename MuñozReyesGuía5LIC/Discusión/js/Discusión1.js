...
btnCalcular.addEventListener("click", function () {
    if (numero.value.length == 0 || elevacion.value.length == 0) {//Validación para que no queden los campos vacíos
        alert('Debe llenar el campo');
        return false;
    } else if (isNaN(numero.value) || isNaN(elevacion.value)) {//Validación para que solo se ingresen números
...
function CalcularPotencia(n, potencia) {
    ...
    this.Elevacion = function () {
        ...
        this.Obtener = function () {//Demostración de los datos ingresados y el resultado obtenido
            contenedor += "<p>El número ingresado es:  " + this.n + "</p>";
            contenedor += "<p>La potencia a elevar es: " + this.potencia + "</p>";
